package cing.client.i18n;

/**
 * Interface to represent the constants contained in resource bundle:
 * 	'/Users/jd/workspace35/cing/src/cing/client/i18n/iCingConstants.properties'.
 */
public interface iCingConstants extends com.google.gwt.i18n.client.Constants {
  
  /**
   * Translated "A".
   * 
   * @return translated "A"
   */
  @DefaultStringValue("A")
  @Key("A")
  String A();

  /**
   * Translated "A zip file with the full report is available as well".
   * 
   * @return translated "A zip file with the full report is available as well"
   */
  @DefaultStringValue("A zip file with the full report is available as well")
  @Key("A_zip_file_with")
  String A_zip_file_with();

  /**
   * Translated "About".
   * 
   * @return translated "About"
   */
  @DefaultStringValue("About")
  @Key("About")
  String About();

  /**
   * Translated "Access key".
   * 
   * @return translated "Access key"
   */
  @DefaultStringValue("Access key")
  @Key("Access_key")
  String Access_key();

  /**
   * Translated "Add another upload.".
   * 
   * @return translated "Add another upload."
   */
  @DefaultStringValue("Add another upload.")
  @Key("Add_another_upl")
  String Add_another_upl();

  /**
   * Translated "Advance to file upload.".
   * 
   * @return translated "Advance to file upload."
   */
  @DefaultStringValue("Advance to file upload.")
  @Key("Advance_to_file")
  String Advance_to_file();

  /**
   * Translated "After a while it might be nice to check here anyway as iCing might have timed out checking. CING might be finished anyhow.".
   * 
   * @return translated "After a while it might be nice to check here anyway as iCing might have timed out checking. CING might be finished anyhow."
   */
  @DefaultStringValue("After a while it might be nice to check here anyway as iCing might have timed out checking. CING might be finished anyhow.")
  @Key("After_a_while_it")
  String After_a_while_it();

  /**
   * Translated "All non-hydrogens".
   * 
   * @return translated "All non-hydrogens"
   */
  @DefaultStringValue("All non-hydrogens")
  @Key("All_non-hydro")
  String All_non_hydro();

  /**
   * Translated "All theoretically possible with NMR".
   * 
   * @return translated "All theoretically possible with NMR"
   */
  @DefaultStringValue("All theoretically possible with NMR")
  @Key("All_theoretic")
  String All_theoretic();

  /**
   * Translated "Back".
   * 
   * @return translated "Back"
   */
  @DefaultStringValue("Back")
  @Key("Back")
  String Back();

  /**
   * Translated "Backbone normality".
   * 
   * @return translated "Backbone normality"
   */
  @DefaultStringValue("Backbone normality")
  @Key("Backbone_norm")
  String Backbone_norm();

  /**
   * Translated "Bad".
   * 
   * @return translated "Bad"
   */
  @DefaultStringValue("Bad")
  @Key("Bad")
  String Bad();

  /**
   * Translated "Begin with file upload.".
   * 
   * @return translated "Begin with file upload."
   */
  @DefaultStringValue("Begin with file upload.")
  @Key("Begin_with_file")
  String Begin_with_file();

  /**
   * Translated "CING".
   * 
   * @return translated "CING"
   */
  @DefaultStringValue("CING")
  @Key("CING")
  String CING();

  /**
   * Translated "CING has finished running. Please find the results".
   * 
   * @return translated "CING has finished running. Please find the results"
   */
  @DefaultStringValue("CING has finished running. Please find the results")
  @Key("CING_has_finishe")
  String CING_has_finishe();

  /**
   * Translated "CING has not finished running.".
   * 
   * @return translated "CING has not finished running."
   */
  @DefaultStringValue("CING has not finished running.")
  @Key("CING_has_not_fin")
  String CING_has_not_fin();

  /**
   * Translated "CING is temporarily down for maintenance; please stay tuned.".
   * 
   * @return translated "CING is temporarily down for maintenance; please stay tuned."
   */
  @DefaultStringValue("CING is temporarily down for maintenance; please stay tuned.")
  @Key("CING_is_temporar")
  String CING_is_temporar();

  /**
   * Translated "CING might have crashed as the server status could not be retrieved. It might also still be running. Please check the results so far.".
   * 
   * @return translated "CING might have crashed as the server status could not be retrieved. It might also still be running. Please check the results so far."
   */
  @DefaultStringValue("CING might have crashed as the server status could not be retrieved. It might also still be running. Please check the results so far.")
  @Key("CING_might_have")
  String CING_might_have();

  /**
   * Translated "Clear".
   * 
   * @return translated "Clear"
   */
  @DefaultStringValue("Clear")
  @Key("Clear")
  String Clear();

  /**
   * Translated "Clears the log window.".
   * 
   * @return translated "Clears the log window."
   */
  @DefaultStringValue("Clears the log window.")
  @Key("Clears_the_log_w")
  String Clears_the_log_w();

  /**
   * Translated "Close".
   * 
   * @return translated "Close"
   */
  @DefaultStringValue("Close")
  @Key("Close")
  String Close();

  /**
   * Translated "Criteria".
   * 
   * @return translated "Criteria"
   */
  @DefaultStringValue("Criteria")
  @Key("Criteria")
  String Criteria();

  /**
   * Translated "Criteria for".
   * 
   * @return translated "Criteria for"
   */
  @DefaultStringValue("Criteria for")
  @Key("Criteria_for")
  String Criteria_for();

  /**
   * Translated "DEBUG".
   * 
   * @return translated "DEBUG"
   */
  @DefaultStringValue("DEBUG")
  @Key("DEBUG")
  String DEBUG();

  /**
   * Translated "Debug".
   * 
   * @return translated "Debug"
   */
  @DefaultStringValue("Debug")
  @Key("Debug")
  String Debug();

  /**
   * Translated "Debug mode".
   * 
   * @return translated "Debug mode"
   */
  @DefaultStringValue("Debug mode")
  @Key("Debug_mode")
  String Debug_mode();

  /**
   * Translated "Detail".
   * 
   * @return translated "Detail"
   */
  @DefaultStringValue("Detail")
  @Key("Detail")
  String Detail();

  /**
   * Translated "ERROR".
   * 
   * @return translated "ERROR"
   */
  @DefaultStringValue("ERROR")
  @Key("ERROR")
  String ERROR();

  /**
   * Translated "E.g.".
   * 
   * @return translated "E.g."
   */
  @DefaultStringValue("E.g.")
  @Key("E_g_")
  String E_g_();

  /**
   * Translated "Edit".
   * 
   * @return translated "Edit"
   */
  @DefaultStringValue("Edit")
  @Key("Edit")
  String Edit();

  /**
   * Translated "Example of this type.".
   * 
   * @return translated "Example of this type."
   */
  @DefaultStringValue("Example of this type.")
  @Key("Eg_of_type")
  String Eg_of_type();

  /**
   * Translated "Ensemble models".
   * 
   * @return translated "Ensemble models"
   */
  @DefaultStringValue("Ensemble models")
  @Key("Ensemble_mode")
  String Ensemble_mode();

  /**
   * Translated "Error".
   * 
   * @return translated "Error"
   */
  @DefaultStringValue("Error")
  @Key("Error")
  String Error();

  /**
   * Translated "Exit".
   * 
   * @return translated "Exit"
   */
  @DefaultStringValue("Exit")
  @Key("Exit")
  String Exit();

  /**
   * Translated "Expect to see the iCing messages scroll by here".
   * 
   * @return translated "Expect to see the iCing messages scroll by here"
   */
  @DefaultStringValue("Expect to see the iCing messages scroll by here")
  @Key("Expect_to_see")
  String Expect_to_see();

  /**
   * Translated "File".
   * 
   * @return translated "File"
   */
  @DefaultStringValue("File")
  @Key("File")
  String File();

  /**
   * Translated "Flag missing coordinates".
   * 
   * @return translated "Flag missing coordinates"
   */
  @DefaultStringValue("Flag missing coordinates")
  @Key("Flag_missing")
  String Flag_missing();

  /**
   * Translated "Go to CING log.".
   * 
   * @return translated "Go to CING log."
   */
  @DefaultStringValue("Go to CING log.")
  @Key("Goto_CING_log")
  String Goto_CING_log();

  /**
   * Translated "Go to CING report.".
   * 
   * @return translated "Go to CING report."
   */
  @DefaultStringValue("Go to CING report.")
  @Key("Goto_CING_report")
  String Goto_CING_report();

  /**
   * Translated "Help".
   * 
   * @return translated "Help"
   */
  @DefaultStringValue("Help")
  @Key("Help")
  String Help();

  /**
   * Translated "Imagery report".
   * 
   * @return translated "Imagery report"
   */
  @DefaultStringValue("Imagery report")
  @Key("Imagery_repor")
  String Imagery_repor();

  /**
   * Translated "Include intra-residual contacts".
   * 
   * @return translated "Include intra-residual contacts"
   */
  @DefaultStringValue("Include intra-residual contacts")
  @Key("Include_intra")
  String Include_intra();

  /**
   * Translated "Log".
   * 
   * @return translated "Log"
   */
  @DefaultStringValue("Log")
  @Key("Log")
  String Log();

  /**
   * Translated "Login".
   * 
   * @return translated "Login"
   */
  @DefaultStringValue("Login")
  @Key("Login")
  String Login();

  /**
   * Translated "Logout".
   * 
   * @return translated "Logout"
   */
  @DefaultStringValue("Logout")
  @Key("Logout")
  String Logout();

  /**
   * Translated "Maximum violation".
   * 
   * @return translated "Maximum violation"
   */
  @DefaultStringValue("Maximum violation")
  @Key("Maximum_violatio")
  String Maximum_violatio();

  /**
   * Translated "NOE completeness".
   * 
   * @return translated "NOE completeness"
   */
  @DefaultStringValue("NOE completeness")
  @Key("NOE_completen")
  String NOE_completen();

  /**
   * Translated "Name".
   * 
   * @return translated "Name"
   */
  @DefaultStringValue("Name")
  @Key("Name")
  String Name();

  /**
   * Translated "New".
   * 
   * @return translated "New"
   */
  @DefaultStringValue("New")
  @Key("New")
  String New();

  /**
   * Translated "Forward".
   * 
   * @return translated "Forward"
   */
  @DefaultStringValue("Forward")
  @Key("Next")
  String Next();

  /**
   * Translated "No Project on server yet.".
   * 
   * @return translated "No Project on server yet."
   */
  @DefaultStringValue("No Project on server yet.")
  @Key("No_Project_on_se")
  String No_Project_on_se();

  /**
   * Translated "No results yet.".
   * 
   * @return translated "No results yet."
   */
  @DefaultStringValue("No results yet.")
  @Key("No_results_yet")
  String No_results_yet();

  /**
   * Translated "Nothing".
   * 
   * @return translated "Nothing"
   */
  @DefaultStringValue("Nothing")
  @Key("Nothing")
  String Nothing();

  /**
   * Translated "Observable atom set".
   * 
   * @return translated "Observable atom set"
   */
  @DefaultStringValue("Observable atom set")
  @Key("Observable_at")
  String Observable_at();

  /**
   * Translated "Omega".
   * 
   * @return translated "Omega"
   */
  @DefaultStringValue("Omega")
  @Key("Omega")
  String Omega();

  /**
   * Translated "once CING is running and you update with the below button.".
   * 
   * @return translated "once CING is running and you update with the below button."
   */
  @DefaultStringValue("once CING is running and you update with the below button.")
  @Key("Once_CING_is_run")
  String Once_CING_is_run();

  /**
   * Translated "Only amides".
   * 
   * @return translated "Only amides"
   */
  @DefaultStringValue("Only amides")
  @Key("Only_amides")
  String Only_amides();

  /**
   * Translated "Only amides and alpha protons".
   * 
   * @return translated "Only amides and alpha protons"
   */
  @DefaultStringValue("Only amides and alpha protons")
  @Key("Only_amides_a")
  String Only_amides_a();

  /**
   * Translated "Options".
   * 
   * @return translated "Options"
   */
  @DefaultStringValue("Options")
  @Key("Options")
  String Options();

  /**
   * Translated "Or visit NRG-CING for a CING validation report of an existing PDB NMR entry.".
   * 
   * @return translated "Or visit NRG-CING for a CING validation report of an existing PDB NMR entry."
   */
  @DefaultStringValue("Or visit NRG-CING for a CING validation report of an existing PDB NMR entry.")
  @Key("Or_visit_NRG_CIN")
  String Or_visit_NRG_CIN();

  /**
   * Translated "Other".
   * 
   * @return translated "Other"
   */
  @DefaultStringValue("Other")
  @Key("Other")
  String Other();

  /**
   * Translated "Output".
   * 
   * @return translated "Output"
   */
  @DefaultStringValue("Output")
  @Key("Output")
  String Output();

  /**
   * Translated "Password".
   * 
   * @return translated "Password"
   */
  @DefaultStringValue("Password")
  @Key("Password")
  String Password();

  /**
   * Translated "Please use a recent version of e.g. Firefox, Chrome or Safari.".
   * 
   * @return translated "Please use a recent version of e.g. Firefox, Chrome or Safari."
   */
  @DefaultStringValue("Please use a recent version of e.g. Firefox, Chrome or Safari.")
  @Key("PleaseUse")
  String PleaseUse();

  /**
   * Translated "Please copy down the url for future reference.".
   * 
   * @return translated "Please copy down the url for future reference."
   */
  @DefaultStringValue("Please copy down the url for future reference.")
  @Key("Please_copy_down")
  String Please_copy_down();

  /**
   * Translated "The filename extension should be:".
   * 
   * @return translated "The filename extension should be:"
   */
  @DefaultStringValue("The filename extension should be:")
  @Key("Please_end_filen")
  String Please_end_filen();

  /**
   * Translated "Please log in; leaving fields empty.".
   * 
   * @return translated "Please log in; leaving fields empty."
   */
  @DefaultStringValue("Please log in; leaving fields empty.")
  @Key("Please_log_in")
  String Please_log_in();

  /**
   * Translated "Please press the button when you are ready.".
   * 
   * @return translated "Please press the button when you are ready."
   */
  @DefaultStringValue("Please press the button when you are ready.")
  @Key("Please_press_the")
  String Please_press_the();

  /**
   * Translated "Poor".
   * 
   * @return translated "Poor"
   */
  @DefaultStringValue("Poor")
  @Key("Poor")
  String Poor();

  /**
   * Translated "Preferences".
   * 
   * @return translated "Preferences"
   */
  @DefaultStringValue("Preferences")
  @Key("Preferences")
  String Preferences();

  /**
   * Translated "Program".
   * 
   * @return translated "Program"
   */
  @DefaultStringValue("Program")
  @Key("Program")
  String Program();

  /**
   * Translated "Remove project data".
   * 
   * @return translated "Remove project data"
   */
  @DefaultStringValue("Remove project data")
  @Key("PurgeProject")
  String PurgeProject();

  /**
   * Translated "Purge the project data on the iCing server.".
   * 
   * @return translated "Purge the project data on the iCing server."
   */
  @DefaultStringValue("Purge the project data on the iCing server.")
  @Key("PurgeProjectTitl")
  String PurgeProjectTitl();

  /**
   * Translated "Removed project data. Please reload iCing to start over.".
   * 
   * @return translated "Removed project data. Please reload iCing to start over."
   */
  @DefaultStringValue("Removed project data. Please reload iCing to start over.")
  @Key("PurgedProject")
  String PurgedProject();

  /**
   * Translated "Randomizing".
   * 
   * @return translated "Randomizing"
   */
  @DefaultStringValue("Randomizing")
  @Key("Randomizing")
  String Randomizing();

  /**
   * Translated "Regenerate".
   * 
   * @return translated "Regenerate"
   */
  @DefaultStringValue("Regenerate")
  @Key("Regenerate")
  String Regenerate();

  /**
   * Translated "Report".
   * 
   * @return translated "Report"
   */
  @DefaultStringValue("Report")
  @Key("Report")
  String Report();

  /**
   * Translated "Residues".
   * 
   * @return translated "Residues"
   */
  @DefaultStringValue("Residues")
  @Key("Residues")
  String Residues();

  /**
   * Translated "Reverse the order of lines in the log.".
   * 
   * @return translated "Reverse the order of lines in the log."
   */
  @DefaultStringValue("Reverse the order of lines in the log.")
  @Key("Reverse_the_orde")
  String Reverse_the_orde();

  /**
   * Translated "RMS violation".
   * 
   * @return translated "RMS violation"
   */
  @DefaultStringValue("RMS violation")
  @Key("Rms_violatio")
  String Rms_violatio();

  /**
   * Translated "Run".
   * 
   * @return translated "Run"
   */
  @DefaultStringValue("Run")
  @Key("Run")
  String Run();

  /**
   * Translated "Run the validation.".
   * 
   * @return translated "Run the validation."
   */
  @DefaultStringValue("Run the validation.")
  @Key("Run_the_validati")
  String Run_the_validati();

  /**
   * Translated "Save".
   * 
   * @return translated "Save"
   */
  @DefaultStringValue("Save")
  @Key("Save")
  String Save();

  /**
   * Translated "Select file(s) to upload.".
   * 
   * @return translated "Select file(s) to upload."
   */
  @DefaultStringValue("Select file(s) to upload.")
  @Key("Select_file_s_")
  String Select_file_s_();

  /**
   * Translated "Set the criteria.".
   * 
   * @return translated "Set the criteria."
   */
  @DefaultStringValue("Set the criteria.")
  @Key("Set_the_criteria")
  String Set_the_criteria();

  /**
   * Translated "Set the options.".
   * 
   * @return translated "Set the options."
   */
  @DefaultStringValue("Set the options.")
  @Key("Set_the_options")
  String Set_the_options();

  /**
   * Translated "Short name like JoeNmr (no special characters)".
   * 
   * @return translated "Short name like JoeNmr (no special characters)"
   */
  @DefaultStringValue("Short name like JoeNmr (no special characters)")
  @Key("Short_name_like")
  String Short_name_like();

  /**
   * Translated "Short secret word like 234567.".
   * 
   * @return translated "Short secret word like 234567."
   */
  @DefaultStringValue("Short secret word like 234567.")
  @Key("Short_secret_wo")
  String Short_secret_wo();

  /**
   * Translated "Silly sound".
   * 
   * @return translated "Silly sound"
   */
  @DefaultStringValue("Silly sound")
  @Key("Silly_sound")
  String Silly_sound();

  /**
   * Translated "Sound".
   * 
   * @return translated "Sound"
   */
  @DefaultStringValue("Sound")
  @Key("Sound")
  String Sound();

  /**
   * Translated "Sound is turned off; change under iCing->Preferences".
   * 
   * @return translated "Sound is turned off; change under iCing->Preferences"
   */
  @DefaultStringValue("Sound is turned off; change under iCing->Preferences")
  @Key("Sound_is_turn")
  String Sound_is_turn();

  /**
   * Translated "Standard".
   * 
   * @return translated "Standard"
   */
  @DefaultStringValue("Standard")
  @Key("Standard")
  String Standard();

  /**
   * Translated "Standard, all individual prochiral groups".
   * 
   * @return translated "Standard, all individual prochiral groups"
   */
  @DefaultStringValue("Standard, all individual prochiral groups")
  @Key("Standard_all")
  String Standard_all();

  /**
   * Translated "Standard, no individual prochiral groups".
   * 
   * @return translated "Standard, no individual prochiral groups"
   */
  @DefaultStringValue("Standard, no individual prochiral groups")
  @Key("Standard_no")
  String Standard_no();

  /**
   * Translated "Start".
   * 
   * @return translated "Start"
   */
  @DefaultStringValue("Start")
  @Key("Start")
  String Start();

  /**
   * Translated "Start new session.".
   * 
   * @return translated "Start new session."
   */
  @DefaultStringValue("Start new session.")
  @Key("Start_new_sessi")
  String Start_new_sessi();

  /**
   * Translated "Submit".
   * 
   * @return translated "Submit"
   */
  @DefaultStringValue("Submit")
  @Key("Submit")
  String Submit();

  /**
   * Translated "Submit to CING server.".
   * 
   * @return translated "Submit to CING server."
   */
  @DefaultStringValue("Submit to CING server.")
  @Key("Submit_to_CING_s")
  String Submit_to_CING_s();

  /**
   * Translated "Subtype".
   * 
   * @return translated "Subtype"
   */
  @DefaultStringValue("Subtype")
  @Key("Subtype")
  String Subtype();

  /**
   * Translated "Tail".
   * 
   * @return translated "Tail"
   */
  @DefaultStringValue("Tail")
  @Key("Tail")
  String Tail();

  /**
   * Translated "This message should not show up.".
   * 
   * @return translated "This message should not show up."
   */
  @DefaultStringValue("This message should not show up.")
  @Key("This_message_sho")
  String This_message_sho();

  /**
   * Translated "Type".
   * 
   * @return translated "Type"
   */
  @DefaultStringValue("Type")
  @Key("Type")
  String Type();

  /**
   * Translated "Uncaught exception".
   * 
   * @return translated "Uncaught exception"
   */
  @DefaultStringValue("Uncaught exception")
  @Key("Uncaught_ex")
  String Uncaught_ex();

  /**
   * Translated "Update".
   * 
   * @return translated "Update"
   */
  @DefaultStringValue("Update")
  @Key("Update")
  String Update();

  /**
   * Translated "Upload".
   * 
   * @return translated "Upload"
   */
  @DefaultStringValue("Upload")
  @Key("Upload")
  String Upload();

  /**
   * Translated "Upload another file".
   * 
   * @return translated "Upload another file"
   */
  @DefaultStringValue("Upload another file")
  @Key("Upload_another_f")
  String Upload_another_f();

  /**
   * Translated "Upload file".
   * 
   * @return translated "Upload file"
   */
  @DefaultStringValue("Upload file")
  @Key("Upload_file")
  String Upload_file();

  /**
   * Translated "Uploading".
   * 
   * @return translated "Uploading"
   */
  @DefaultStringValue("Uploading")
  @Key("Uploading")
  String Uploading();

  /**
   * Translated "Validation settings file must be named:".
   * 
   * @return translated "Validation settings file must be named:"
   */
  @DefaultStringValue("Validation settings file must be named:")
  @Key("Validation_setti")
  String Validation_setti();

  /**
   * Translated "Verbosity".
   * 
   * @return translated "Verbosity"
   */
  @DefaultStringValue("Verbosity")
  @Key("Verbosity")
  String Verbosity();

  /**
   * Translated "Verbosity may be controlled from iCing->Preferences".
   * 
   * @return translated "Verbosity may be controlled from iCing->Preferences"
   */
  @DefaultStringValue("Verbosity may be controlled from iCing->Preferences")
  @Key("Verbosity_may")
  String Verbosity_may();

  /**
   * Translated "View".
   * 
   * @return translated "View"
   */
  @DefaultStringValue("View")
  @Key("View")
  String View();

  /**
   * Translated "WARNING".
   * 
   * @return translated "WARNING"
   */
  @DefaultStringValue("WARNING")
  @Key("WARNING")
  String WARNING();

  /**
   * Translated "Warning".
   * 
   * @return translated "Warning"
   */
  @DefaultStringValue("Warning")
  @Key("Warning")
  String Warning();

  /**
   * Translated "Welcome".
   * 
   * @return translated "Welcome"
   */
  @DefaultStringValue("Welcome")
  @Key("Welcome")
  String Welcome();

  /**
   * Translated "Welcome to".
   * 
   * @return translated "Welcome to"
   */
  @DefaultStringValue("Welcome to")
  @Key("Welcome_to")
  String Welcome_to();

  /**
   * Translated "You must select a file!".
   * 
   * @return translated "You must select a file!"
   */
  @DefaultStringValue("You must select a file!")
  @Key("You_must_sele")
  String You_must_sele();

  /**
   * Translated "and".
   * 
   * @return translated "and"
   */
  @DefaultStringValue("and")
  @Key("and")
  String and();

  /**
   * Translated "directory".
   * 
   * @return translated "directory"
   */
  @DefaultStringValue("directory")
  @Key("directory")
  String directory();

  /**
   * Translated "For help with iCing, please refer to the help listed at CING's homepage".
   * 
   * @return translated "For help with iCing, please refer to the help listed at CING's homepage"
   */
  @DefaultStringValue("For help with iCing, please refer to the help listed at CING's homepage")
  @Key("for_help")
  String for_help();

  /**
   * Translated "Procheck G factor".
   * 
   * @return translated "Procheck G factor"
   */
  @DefaultStringValue("Procheck G factor")
  @Key("gFactor")
  String gFactor();

  /**
   * Translated "here".
   * 
   * @return translated "here"
   */
  @DefaultStringValue("here")
  @Key("here")
  String here();

  /**
   * Translated "iCing".
   * 
   * @return translated "iCing"
   */
  @DefaultStringValue("iCing")
  @Key("iCing")
  String iCing();

  /**
   * Translated "Validation of NMR structures".
   * 
   * @return translated "Validation of NMR structures"
   */
  @DefaultStringValue("Validation of NMR structures")
  @Key("iCing_subtitle")
  String iCing_subtitle();

  /**
   * Translated "in which results are being created that may be consulted in the meanwhile. You may also switch to".
   * 
   * @return translated "in which results are being created that may be consulted in the meanwhile. You may also switch to"
   */
  @DefaultStringValue("in which results are being created that may be consulted in the meanwhile. You may also switch to")
  @Key("in_which_results")
  String in_which_results();

  /**
   * Translated "is part of the".
   * 
   * @return translated "is part of the"
   */
  @DefaultStringValue("is part of the")
  @Key("is_part")
  String is_part();

  /**
   * Translated "none".
   * 
   * @return translated "none"
   */
  @DefaultStringValue("none")
  @Key("none")
  String none();

  /**
   * Translated "occurrences in db".
   * 
   * @return translated "occurrences in db"
   */
  @DefaultStringValue("occurrences in db")
  @Key("occurrences_in")
  String occurrences_in();

  /**
   * Translated "per residue".
   * 
   * @return translated "per residue"
   */
  @DefaultStringValue("per residue")
  @Key("per_residue")
  String per_residue();

  /**
   * Translated "plot".
   * 
   * @return translated "plot"
   */
  @DefaultStringValue("plot")
  @Key("plot")
  String plot();

  /**
   * Translated "residue sigmas".
   * 
   * @return translated "residue sigmas"
   */
  @DefaultStringValue("residue sigmas")
  @Key("residue_sigma")
  String residue_sigma();

  /**
   * Translated "software".
   * 
   * @return translated "software"
   */
  @DefaultStringValue("software")
  @Key("software")
  String software();

  /**
   * Translated "3D".
   * 
   * @return translated "3D"
   */
  @DefaultStringValue("3D")
  @Key("threeD")
  String threeD();

  /**
   * Translated "uploaded".
   * 
   * @return translated "uploaded"
   */
  @DefaultStringValue("uploaded")
  @Key("uploaded")
  String uploaded();

  /**
   * Translated "version".
   * 
   * @return translated "version"
   */
  @DefaultStringValue("version")
  @Key("version")
  String version();

  /**
   * Translated "Unfortunately, iCing does not work on this browser".
   * 
   * @return translated "Unfortunately, iCing does not work on this browser"
   */
  @DefaultStringValue("Unfortunately, iCing does not work on this browser")
  @Key("warningBrowser")
  String warningBrowser();
}
