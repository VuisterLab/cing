...
from cing.PluginCode.pdb import MatchGame
switchOutput(False)
...
"""
    ...
    Steps:
    - Parse the ccpn file using the CCPN api (__init__).
    ...
    - Generate a molecule from chn,res's
...
class Ccpn:
    ...
    def _getCcpnNmrProject(self):
        ...
        if self.ccpnProject.currentNmrProject:
            self.ccpnNmrProject = self.ccpnProject.currentNmrProject
        elif self.ccpnProject.nmrProjects:
            self.ccpnNmrProject =  self.ccpnProject.findFirstNmrProject()            
        return self.ccpnNmrProject  
    ...
    #        if coords:
            ...
            for ccpnResidue in ccpnChain.sortedResidues():
                resName = ccpnResidue.chemCompVar.chemComp.code3Letter    

                # JFD adds: In 1ai0 the only occurrence of the 'CING' string in any of the CCPN files was:
    #    <CHEM.NamingSystem _ID="_36" name="CING">
    #    </CHEM.NamingSystem>
                # The id then doesn't occur anymore so my assumption is that the CING names are not (yet)
                # included in CCPN reference data for the uncommon residues.
                # In the standard residue library e.g. for Arg: I find CING:
    #    <CHEM.NamingSystem _ID="_144" name="CING">
    #      <CHEM.NamingSystem.chemCompSysNames>
    #        <CHEM.ChemCompSysName _ID="_112" sysName="ARG">
    #          <CHEM.ChemCompSysName.specificChemCompVars> _42 _33 _37 _38 _41 _45 _34 _44 _49</CHEM.ChemCompSysName.specificChemCompVars>
    #        </CHEM.ChemCompSysName>
    #        <CHEM.ChemCompSysName _ID="_109" sysName="ARGx">
    #          <CHEM.ChemCompSysName.specificChemCompVars> _48 _32 _35 _39 _40 _43 _36 _46 _47</CHEM.ChemCompSysName.specificChemCompVars>
    #        </CHEM.ChemCompSysName>
    #      </CHEM.NamingSystem.chemCompSysNames>
    #    </CHEM.NamingSystem>
                # Since it will be very hard to keep CCPN and CING synchronized to know of each others names a simpler
                # strategy is attempted. Use the             
    
                residue = chain.addResidue(resName, ccpnResidue.seqCode)
                # Make mutual linkages between Ccpn and Cing objects
                ...
                self._matchAtom2Cing(ccpnResidue, ccpnChainLetter, ccpnCoordResidueList, resName)
            # end for
        # end for
        return self.molecule
    # end def _match2Cing
    
    def _matchAtom2Cing(self, ccpnResidue, ccpnChainLetter, ccpnCoordResidueList, resName):
        ...
        for ccpnAtom in ccpnResidue.sortedAtoms():
            ...
            cingNameTuple = (INTERNAL, ccpnChainLetter, ccpnResSeq, atomName)
            atom = self.molecule.decodeNameTuple(cingNameTuple)
    
            if not atom:
                NTdebug('Atom %s in CCPN not found in CING %s. Creating non-standard atom' % (cingNameTuple, self.molecule))
                cingResNameTuple = (INTERNAL, ccpnChainLetter, ccpnResSeq, None)
                res = self.molecule.decodeNameTuple(cingResNameTuple)
                if not res:
                    NTcodeerror('No residue found in CING for tuple %s. Creating non-standard atom' % cingResNameTuple)
                    continue
