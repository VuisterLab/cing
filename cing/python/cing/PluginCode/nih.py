...
#    cingPaths.talosPlus = 'talos+'
    ...
    if not parseOnly:
        ...
        # Check for existence of the output files
        ...
        if talosDefs.predFile == None:
            nTerror("RunTalosPlus: Failed to find pred.tab file")
            return True
        #end if
        ...
        if talosDefs.predSSFile == None:
            nTerror("RunTalosPlus: Failed to find predSS.tab or pred.ss.tab file")
            return True
        #end if
...
def importTalosPlus( project, tmp=None ):
    ...
    talosDefs = project.getStatusDict('talosPlus', **talosDefaults())
...
#        nTdebug("saveTalosPlus: No talos+ was run")
    ...
    path = project.path() / directories.plugins
    if not path.exists():
        path.makedirs()
    ...
    talosDefs.savedInData = True
    talosDefs.savedAsSml = True
    talosDefs.version = version
...
def restoreTalosPlus( project, tmp=None ):
    ...
    if not talosDefs.savedInData:
        # Old location in original talosPlus directory
        path = project.validationPath( talosDefs.directory)
    else:
        path = project.path() / directories.plugins
    #end if
    ...
    # end if
    # Restore the data
    nTmessage('==> Restoring talos+ results from %s (code version %s)', smlFile, talosDefs.version)
