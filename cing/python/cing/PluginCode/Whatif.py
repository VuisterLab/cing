...
#    nTdebug("Removing temporary files generated by What If")
    try:
        # Don't remove pdbout.txt but the pdbout files for each model 
        ...
        for extension in 'pdbout_???.txt *.script *.eps *.pdb *.OUT *.LOG *.DAT *.SCC *.sty *.FIG *.ATM DAVADRUG.* PRODRUG.*'.split():
