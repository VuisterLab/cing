...
class nmrPipeTable( NTstruct ):
    ...
    def addColumn( self, name, fmt = "%s", default=None ):
        ...
        """
        if name in self:
            NTerror('ERROR nmrPipeTable.addColumn: column "%s" already exists\n', name )
            return None
        #end if
    ...
    def hideColumn( self, *cNames ):
        ...
        for c in cNames:
            if not c in self:
                NTerror('ERROR nmrPipeTable.hideColumn: column "%s" not defined\n', c)
            else:
                self[c].hide = True
            #end if
        #end for
    ...
    def showColumn( self, *cNames ):
        ...
        for c in cNames:
            if not c in self:
                NTerror('ERROR nmrPipeTable.showColumn: column "%s" not defined\n', c)
            else:
                self[c].hide = False
            #end if
        #end for
