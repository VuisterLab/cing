...
class GenClient {
    ...
    // public static int verbosity = verbosityDebug; DON'T CHANGE HERE; USE
    // iCing.java#onModuleLoad() call to GenClient.setVerbosityToDebug();
    ...
    public static void doOutOfMemoryExit() {
        ...
        message += "specify this when starting the Java virtual machine, like:"
                + eol;
        message += "   java -Xmx1500m Wattos.Utils.General" + eol;
        message += "to set the maximum memory of the heap to 512 Mb." + eol
                + eol;
        showError(message);
    ...
    public static void doCodeBugExit(String message, String lastKnownPosition,
            Throwable t, Class c) {
        showCodeBug(message, lastKnownPosition, t, c);
        doExit(EXIT_STATUS_CODE_ERROR);
    }

    public static void showCodeBug(String message, String lastKnownPosition,
            Throwable t, Class c) {
    ...
    public static void showError(String message, String lastKnownPosition,
            Throwable t, Class cl) {
    ...
    public static void showWarning(String message, String lastKnownPosition,
            Throwable t, Class c) {
        if (verbosity < verbosityWarning) {
            return;
        }
        String prefix = "<font color=\"orange\">" + constants.WARNING()
                + "</font>";
        if (lastKnownPosition != null) {
            message += eol + constants.WARNING()
                    + ": ran into a uncommon situation at: ";
            if (c != null) {
                message += c.getName() + ' ';
            } else {
                message += "unknown class, ";
    ...
    public static void showDebug(String message, String lastKnownPosition,
            Throwable t, Class c) {
     ...
     *            operation.
     */
    public static void rotateCollectionToFirst(ArrayList al,
    ...
    /**
     * public static final int verbosityNothing = 0; // Even errors will be
     * suppressed public static final int verbosityError = 1; // show only
     * errors public static final int verbosityWarning = 2; // show errors and
     * warnings public static final int verbosityOutput = 3; // and regular
     * output DEFAULT public static final int verbosityDetail = 4; // show more
     * details public static final int verbosityDebug = 5; // add debugging info
     * (not
     *
     * verbosityNothing = 0 # Even errors will be suppressed verbosityError = 1
     * # show only errors verbosityWarning = 2 # show errors and warnings
     * verbosityOutput = 3 # and regular output DEFAULT verbosityDetail = 4 #
     * show more details verbosityDebug = 9 # add debugging info (not
     * recommended for casual user)
     *
     * @param verbosity
     * @return
