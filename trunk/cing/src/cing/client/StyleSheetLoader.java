...
public class StyleSheetLoader {
	 ...
	 * when a new style sheet has finished loading. The widget has a natural
	 * width of 0px, but when the style sheet is loaded, the width changes to
	 * 5px. The style sheet should contain a style definition that is passed
	...
	private static class StyleTesterTimer extends Timer {
		...
		public void run() {
			...
			if (refWidget.getOffsetWidth() > 0) {
				...
				// has
				// enough time to parse the styles. Otherwise, we'll get weird
				// styling
				// issues.
	 ...
	 * The style sheet should contain a style definition called refStyleName
	 * that defines a height and width greater than 0px.
	 * 
	 * @param href
	 *            the url of the style sheet
	 * @param refStyleName
	...
	/**
	 * Detect when a style sheet has loaded by placing an element on the page
	 * that is affected by a rule in the style sheet, as described in
	 * {@link #loadStyleSheet(String, String, Command)}. When the style sheet
	 * has loaded, the callback will be executed.
	 * 
	 * @param refStyleName
	 *            the style name of the reference element
	 * @param callback
