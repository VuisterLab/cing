...
def appendDeepByKeys( d, value, *keyList ):
    """TODO: remove this method and setDeepByKeys when
    pickling the NTdict works.

    Append value to arbitrary deep list.
    If value is a (subclass of) list the append individual values.
    The essence here is silence.
    keyList needs to have at least one key.
    Return None on success and True on error.
    """
...
#  NTdebug("Now in appendDeepByKeys with keyList: %s", `keyList`)
    ...
    if lk == 1:
        ...
        if isinstance(value,list):
            ...
            d[k].append(value) # No extra checks done here for speed purposes.
        return
